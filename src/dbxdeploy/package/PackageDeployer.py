...
class PackageDeployer:
    ...
    def deploy(self, packageMetadata: PackageMetadata, resultOutputFile):
        ...
        with packagePath.open('rb') as file:
            ...
            if self.__targetPathsResolver.hasPackageUploadPathForCurrent():
                ...
                outputPath = whlCurrentPath

        self.__logger.info(f'App package uploaded to {outputPath}')

        f = open(resultOutputFile, "w")
        f.write(outputPath)
        f.close()
